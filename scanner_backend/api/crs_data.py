CRS_MAPPING = {
  '11110': {
    'ODS 4 Educación de calidad': ['4.5. Eliminar disparidades género en educación y acceso personas vulnerables']
  },
  '11120': {
    'ODS 4 Educación de calidad': ['4.5. Eliminar disparidades género en educación y acceso personas vulnerables', '4.A. Instalaciones escolares respondan necesidades niños y personas con discapacidad']
  },
  '11130': {
    'ODS 4 Educación de calidad': ['4.5. Eliminar disparidades género en educación y acceso personas vulnerables', '4.C. Aumentar oferta de maestros calificados']
  },
  '11182': {
    'ODS 4 Educación de calidad': ['4.5. Eliminar disparidades género en educación y acceso personas vulnerables']
  },
  '11220': {
    'ODS 4 Educación de calidad': ['4.1. Enseñanza gratuita, equitativa y de calidad', '4.5. Eliminar disparidades género en educación y acceso personas vulnerables', '4.6. Garantizar que todos los jóvenes y al menos una proporción sustancial de adultos tengan competencias de lectura, escritura y aritmética']
  },
  '11230': {
    'ODS 4 Educación de calidad': ['4.5. Eliminar disparidades género en educación y acceso personas vulnerables', '4.6. Garantizar que todos los jóvenes y al menos una proporción sustancial de adultos tengan competencias de lectura, escritura y aritmética']
  },
  '11240': {
    'ODS 4 Educación de calidad': ['4.2. Todos acceso servicios atención primera infancia y enseñanza preescolar', '4.5. Eliminar disparidades género en educación y acceso personas vulnerables', '4.6. Garantizar que todos los jóvenes y al menos una proporción sustancial de adultos tengan competencias de lectura, escritura y aritmética']
  },
  '11320': {
    'ODS 4 Educación de calidad': ['4.1. Enseñanza gratuita, equitativa y de calidad', '4.4. Aumentar competencias técnicas y profesionales para acceso empleo', '4.5. Eliminar disparidades género en educación y acceso personas vulnerables']
  },
  '11330': {
    'ODS 4 Educación de calidad': ['4.3. Acceso igual todos en formación técnica, profesional y superior', '4.4. Aumentar competencias técnicas y profesionales para acceso empleo', '4.5. Eliminar disparidades género en educación y acceso personas vulnerables'],
    'ODS 8 Trabajo decente y crecimiento económico': ['8.6. Reducir nº jóvenes que no trabajan, no cursan estudios ni capacitación']
  },
  '11420': {
    'ODS 4 Educación de calidad': ['4.B. Aumentar nº becas para países en desarrollo']
  },
  '11430': {
    'ODS 4 Educación de calidad': ['4.3. Acceso igual todos en formación técnica, profesional y superior', '4.5. Eliminar disparidades género en educación y acceso personas vulnerables']
  },
  '12110': {
    'ODS 3 Salud y bienestar': ['3.8. Contribuir a la cobertura sanitaria universal']
  },
  '12181': {
    'ODS 3 Salud y bienestar': ['3.C. Financiación salud y personal sanitario en países en desarrollo']
  },
  '12182': {
    'ODS 3 Salud y bienestar': ['3.B. Investigación y desarrollo de vacunas y medicamentos']
  },
  '12191': {
    'ODS 3 Salud y bienestar': ['3.5. Prevención y el tratamiento abuso sustancias adictivas', '3.A. Fortalecer la aplicación del Convenio Marco de la OMS sobre el Control del Tabaco']
  },
  '12220': {
    'ODS 3 Salud y bienestar': ['3.2. Fin muertes evitables de recién nacidos y < 5 años', '3.B. Investigación y desarrollo de vacunas y medicamentos']
  },
  '12240': {
    'ODS 2 Hambre cero': ['2.1. Fin al hambre y asegurar acceso alimentación', '2.2. Fin a todas las formas de malnutrición']
  },
  '12250': {
    'ODS 3 Salud y bienestar': ['3.3. Fin epidemias SIDA, tuberculosis, malaria y enf. tropicales']
  },
  '12262': {
    'ODS 3 Salud y bienestar': ['3.3. Fin epidemias SIDA, tuberculosis, malaria y enf. tropicales']
  },
  '12263': {
    'ODS 3 Salud y bienestar': ['3.3. Fin epidemias SIDA, tuberculosis, malaria y enf. tropicales']
  },
  '12281': {
    'ODS 3 Salud y bienestar': ['3.C. Financiación salud y personal sanitario en países en desarrollo']
  },
  '13010': {
    'ODS 1 Fin de la pobreza': ['16.9. Acceso a identidad jurídica para todos, registro de nacimientos'],
    'ODS 3 Salud y bienestar': ['3.7. Acceso universal servicios salud sexual y reproductiva'],
    'ODS 5 Igualdad de género': ['5.6. Acceso universal a salud sexual y reproductiva y dchos reproductivos']
  },
  '13020': {
    'ODS 3 Salud y bienestar': ['3.1. Reducir mortalidad materna a < 70 por 100.000 nacidos vivos', '3.7. Acceso universal servicios salud sexual y reproductiva'],
    'ODS 5 Igualdad de género': ['5.6. Acceso universal a salud sexual y reproductiva y dchos reproductivos']
  },
  '13030': {
    'ODS 3 Salud y bienestar': ['3.7. Acceso universal servicios salud sexual y reproductiva'],
    'ODS 5 Igualdad de género': ['5.6. Acceso universal a salud sexual y reproductiva y dchos reproductivos']
  },
  '13040': {
    'ODS 3 Salud y bienestar': ['3.3. Fin epidemias SIDA, tuberculosis, malaria y enf. tropicales', '3.7. Acceso universal servicios salud sexual y reproductiva'],
    'ODS 5 Igualdad de género': ['5.6. Acceso universal a salud sexual y reproductiva y dchos reproductivos']
  },
  '13081': {
    'ODS 3 Salud y bienestar': ['3.7. Acceso universal servicios salud sexual y reproductiva', '3.C. Financiación salud y personal sanitario en países en desarrollo'],
    'ODS 5 Igualdad de género': ['5.6. Acceso universal a salud sexual y reproductiva y dchos reproductivos']
  },
  '14010': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres'],
    'ODS 6 Agua limpia y saneamiento': ['6.3. Mejorar calidad agua con reducción contaminación', '6.5. Mejorar la gestión integrada de recursos hidraulicos', '6.6. Proteger y restablecer los ecosistemas relacionados con el agua', '6.A. Ampliar cooperación en programas de agua y saneamiento']
  },
  '14015': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres'],
    'ODS 3 Salud y bienestar': ['3.9. Reducir nº muertes/enfermedades por productos químicos'],
    'ODS 6 Agua limpia y saneamiento': ['6.A. Ampliar cooperación en programas de agua y saneamiento']
  },
  '14020': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres'],
    'ODS 6 Agua limpia y saneamiento': ['6.A. Ampliar cooperación en programas de agua y saneamiento']
  },
  '14021': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres'],
    'ODS 6 Agua limpia y saneamiento': ['6.1. Acceso universal y equitativo al agua potable', '6.A. Ampliar cooperación en programas de agua y saneamiento']
  },
  '14022': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres'],
    'ODS 6 Agua limpia y saneamiento': ['6.2. Lograr acceso equitativo a servicios de saneamiento e higiene', '6.3. Mejorar calidad agua con reducción contaminación', '6.A. Ampliar cooperación en programas de agua y saneamiento']
  },
  '14030': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres'],
    'ODS 6 Agua limpia y saneamiento': ['6.A. Ampliar cooperación en programas de agua y saneamiento']
  },
  '14031': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres'],
    'ODS 6 Agua limpia y saneamiento': ['6.1. Acceso universal y equitativo al agua potable', '6.A. Ampliar cooperación en programas de agua y saneamiento']
  },
  '14032': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres'],
    'ODS 6 Agua limpia y saneamiento': ['6.2. Lograr acceso equitativo a servicios de saneamiento e higiene', '6.A. Ampliar cooperación en programas de agua y saneamiento']
  },
  '14040': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres'],
    'ODS 6 Agua limpia y saneamiento': ['6.A. Ampliar cooperación en programas de agua y saneamiento']
  },
  '14050': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres', '11.6. Reducir impacto ambiental negativo de las ciudades', '12.4. Gestión eco productos químicos y desechos'],
    'ODS 6 Agua limpia y saneamiento': ['6.A. Ampliar cooperación en programas de agua y saneamiento']
  },
  '14081': {
    'ODS 1 Fin de la pobreza': ['11.5. Reducir muertes y afectadas por desastres'],
    'ODS 6 Agua limpia y saneamiento': ['6.A. Ampliar cooperación en programas de agua y saneamiento', '6.B. Participación comunidades locales en gestión agua y saneamiento']
  },
  '15110': {
    'ODS 1 Fin de la pobreza': ['16.6. Crear instituciones eficaces, responsables y transparentes']
  },
  '15113': {
    'ODS 1 Fin de la pobreza': ['16.5. Reducir sustancialmente la corrupción y el soborno en todas sus formas']
  },
  '15114': {
    'ODS 1 Fin de la pobreza': ['17.1. Fortalecer la movilización de recursos internos']
  },
  '15130': {
    'ODS 1 Fin de la pobreza': ['16.3. Promover el Estado de derecho e igualdad acceso a justicia']
  },
  '15150': {
    'ODS 1 Fin de la pobreza': ['16.7. Toma decisiones inclusivas, participativas y representativas']
  },
  '15160': {
    'ODS 1 Fin de la pobreza': ['16.2. Fin maltrato, explotación, trata, tortura y violencia contra niños', '16.10. Acceso público información y proteger libertades'],
    'ODS 5 Igualdad de género': ['5.3. Eliminar matrimonio infantil, forzado y mutilación genital femenina'],
    'ODS 8 Trabajo decente y crecimiento económico': ['8.7. Adoptar medidas para erradicar el trabajo forzoso']
  },
  '15170': {
    'ODS 5 Igualdad de género': ['5.5. Plena y efectiva participación de las mujeres e igualdad oportunidaes', '5.A. Reformas mujeres dcho a recursos econom. condiciones igualdad']
  },
  '15180': {
    'ODS 5 Igualdad de género': ['5.2. Eliminar toda violencia contra todas las mujeres y niñas']
  },
  '15210': {
    'ODS 1 Fin de la pobreza': ['16.A. Fortalecer las instituciones nacionales pertinentes']
  },
  '15220': {
    'ODS 1 Fin de la pobreza': ['16.A. Fortalecer las instituciones nacionales pertinentes']
  },
  '15230': {
    'ODS 1 Fin de la pobreza': ['16.A. Fortalecer las instituciones nacionales pertinentes']
  },
  '15240': {
    'ODS 1 Fin de la pobreza': ['16.A. Fortalecer las instituciones nacionales pertinentes']
  },
  '15250': {
    'ODS 1 Fin de la pobreza': ['16.A. Fortalecer las instituciones nacionales pertinentes']
  },
  '15261': {
    'ODS 1 Fin de la pobreza': ['16.A. Fortalecer las instituciones nacionales pertinentes']
  },
  '16010': {
    'ODS 1 Fin de la pobreza': ['1.3. Sistemas y medidas apropiadas de protección social']
  },
  '16020': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.5. Lograr empleo digno y de calidad', '8.6. Reducir nº jóvenes que no trabajan, no cursan estudios ni capacitación', '8.8. Proteger dchos laborales y promover entorno de trabajo seguro', '8.B. Desarrollar y poner en marcha una estrategia mundial para el empleo de los jóvenes']
  },
  '16030': {
    'ODS 1 Fin de la pobreza': ['11.1. Asegurar acceso a viviendas y servicios básicos']
  },
  '16040': {
    'ODS 1 Fin de la pobreza': ['11.1. Asegurar acceso a viviendas y servicios básicos']
  },
  '16062': {
    'ODS 1 Fin de la pobreza': ['17.18. Mejorar apoyo fomento capacidad a países en desarrollo', '17.19. Elaborar indicadores midan progresos desarrollo sostenible']
  },
  '21010': {
    'ODS 1 Fin de la pobreza': ['11.2. Acceso sist. transporte seguros, asequibles, accesibles, sostenibles']
  },
  '21020': {
    'ODS 1 Fin de la pobreza': ['11.2. Acceso sist. transporte seguros, asequibles, accesibles, sostenibles'],
    'ODS 3 Salud y bienestar': ['3.6. Reducir a la mitad el número de muertes y lesiones causadas por accidentes de tráfico en el mundo']
  },
  '21030': {
    'ODS 1 Fin de la pobreza': ['11.2. Acceso sist. transporte seguros, asequibles, accesibles, sostenibles']
  },
  '21040': {
    'ODS 1 Fin de la pobreza': ['11.2. Acceso sist. transporte seguros, asequibles, accesibles, sostenibles']
  },
  '21050': {
    'ODS 1 Fin de la pobreza': ['11.2. Acceso sist. transporte seguros, asequibles, accesibles, sostenibles']
  },
  '21061': {
    'ODS 1 Fin de la pobreza': ['11.2. Acceso sist. transporte seguros, asequibles, accesibles, sostenibles']
  },
  '21081': {
    'ODS 1 Fin de la pobreza': ['11.2. Acceso sist. transporte seguros, asequibles, accesibles, sostenibles']
  },
  '22040': {
    'ODS 1 Fin de la pobreza': ['17.8. Banco tecnología y apoyo a ciencia, tecnología e innovación'],
    'ODS 5 Igualdad de género': ['5.B. Mejorar el uso de la tecnología instrumental, en particular la tecnología de la información y las comunicaciones'],
    'ODS 9 Industria, innovación e infraestructura': ['9.C. Más acceso tec. información y comunicaciones e Internet']
  },
  '23110': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos', '7.3 . Duplicar tasa mundial de mejora eficiencia energética']
  },
  '23181': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23182': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos', '7.A . Más cooperación investigación y tecnologías energéticas no contaminantes']
  },
  '23183': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23210': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos', '7.2 . Aumentar la proporción de energias renovables', '7.B. Ampliar la infraestructura y mejorar la tecnología para prestar servicios de energía modernos y sostenibles']
  },
  '23220': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos', '7.2 . Aumentar la proporción de energias renovables', '7.B. Ampliar la infraestructura y mejorar la tecnología para prestar servicios de energía modernos y sostenibles']
  },
  '23230': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos', '7.2 . Aumentar la proporción de energias renovables', '7.B. Ampliar la infraestructura y mejorar la tecnología para prestar servicios de energía modernos y sostenibles']
  },
  '23240': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos', '7.B. Ampliar la infraestructura y mejorar la tecnología para prestar servicios de energía modernos y sostenibles']
  },
  '23250': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos', '7.2 . Aumentar la proporción de energias renovables', '7.B. Ampliar la infraestructura y mejorar la tecnología para prestar servicios de energía modernos y sostenibles']
  },
  '23260': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos', '7.B. Ampliar la infraestructura y mejorar la tecnología para prestar servicios de energía modernos y sostenibles']
  },
  '23270': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos', '7.2 . Aumentar la proporción de energias renovables', '7.B. Ampliar la infraestructura y mejorar la tecnología para prestar servicios de energía modernos y sostenibles']
  },
  '23310': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23320': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23330': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23340': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23350': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23360': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23410': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23510': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23610': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23620': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23630': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '23640': {
    'ODS 7 Energía asequible y no contaminante': ['7.1. Acceso servicios energía asequibles, confiables y modernos']
  },
  '24010': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.10. Fortalecer la capacidad de las instituciones financieras nacionales'],
    'ODS 9 Industria, innovación e infraestructura': ['9.3. Acceso peq. empresas serv. financieros y mercados']
  },
  '24020': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.10. Fortalecer la capacidad de las instituciones financieras nacionales'],
    'ODS 9 Industria, innovación e infraestructura': ['9.3. Acceso peq. empresas serv. financieros y mercados']
  },
  '24030': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.10. Fortalecer la capacidad de las instituciones financieras nacionales'],
    'ODS 9 Industria, innovación e infraestructura': ['9.3. Acceso peq. empresas serv. financieros y mercados']
  },
  '24040': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.10. Fortalecer la capacidad de las instituciones financieras nacionales'],
    'ODS 9 Industria, innovación e infraestructura': ['9.3. Acceso peq. empresas serv. financieros y mercados']
  },
  '24050': {
    'ODS 1 Fin de la pobreza': ['10.C. Reducir a menos del 3% los costos de transacción de las remesas de los migrantes']
  },
  '24081': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.10. Fortalecer la capacidad de las instituciones financieras nacionales'],
    'ODS 9 Industria, innovación e infraestructura': ['9.3. Acceso peq. empresas serv. financieros y mercados']
  },
  '31110': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa', '2.B. Corregir y prevenir las restricciones y distorsiones comerciales en los mercados agropecuarios mundiales']
  },
  '31120': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31130': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31140': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31150': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31161': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31162': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31163': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31164': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31165': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31166': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31181': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31182': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa', '2.A. Más inversiones infraestructura rural, investigación agrícola']
  },
  '31191': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa', '2.C. Adoptar medidas para asegurar el buen funcionamiento de los mercados de productos básicos alimentarios y sus derivados y facilitar acceso a información sobre los mercados']
  },
  '31192': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa'],
    'ODS 3 Salud y bienestar': ['3.9. Reducir nº muertes/enfermedades por productos químicos']
  },
  '31193': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31194': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31195': {
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos', '2.4. Sostenibilidad producción de alimentos', '2.5. Mantener la diversidad genética de las semillas, las plantas cultivadas y los animales de granja y domesticados y sus especies silvestres conexa']
  },
  '31220': {
    'ODS 1 Fin de la pobreza': ['15.2. Promover gestión sostenible de los bosques', '15.B. Movilizar un volumen apreciable de recursos procedentes de todas las fuentes y a todos los niveles']
  },
  '31310': {
    'ODS 1 Fin de la pobreza': ['14.4. Regular eficazmente la explotación pesquera y poner fin a la pesca excesiva', '14.7. Aumentar los beneficios económicos que los pequeños Estados insulares en desarrollo y los países menos adelantados reciben del uso sostenible de los recursos marinos'],
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos']
  },
  '31320': {
    'ODS 1 Fin de la pobreza': ['14.4. Regular eficazmente la explotación pesquera y poner fin a la pesca excesiva', '14.7. Aumentar los beneficios económicos que los pequeños Estados insulares en desarrollo y los países menos adelantados reciben del uso sostenible de los recursos marinos'],
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos']
  },
  '31381': {
    'ODS 1 Fin de la pobreza': ['14.4. Regular eficazmente la explotación pesquera y poner fin a la pesca excesiva', '14.7. Aumentar los beneficios económicos que los pequeños Estados insulares en desarrollo y los países menos adelantados reciben del uso sostenible de los recursos marinos'],
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos']
  },
  '31382': {
    'ODS 1 Fin de la pobreza': ['14.4. Regular eficazmente la explotación pesquera y poner fin a la pesca excesiva', '14.7. Aumentar los beneficios económicos que los pequeños Estados insulares en desarrollo y los países menos adelantados reciben del uso sostenible de los recursos marinos'],
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos']
  },
  '31391': {
    'ODS 1 Fin de la pobreza': ['14.4. Regular eficazmente la explotación pesquera y poner fin a la pesca excesiva', '14.7. Aumentar los beneficios económicos que los pequeños Estados insulares en desarrollo y los países menos adelantados reciben del uso sostenible de los recursos marinos'],
    'ODS 2 Hambre cero': ['2.3. Mayor productividad agrícola e ingresos productores alimentos']
  },
  '32110': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.3. Promover políticas para desarrollo actividades productivas'],
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32120': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.3. Promover políticas para desarrollo actividades productivas'],
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32130': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.3. Promover políticas para desarrollo actividades productivas'],
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32140': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.3. Promover políticas para desarrollo actividades productivas'],
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32161': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32162': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32163': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32164': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32165': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32166': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32167': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32168': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32169': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32170': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32171': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32172': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible']
  },
  '32182': {
    'ODS 9 Industria, innovación e infraestructura': ['9.2. Promover una industrialización inclusiva y sostenible', '9.5. Más investigación científica y capacidad tec. industrial']
  },
  '32210': {
    'ODS 1 Fin de la pobreza': ['12.C. Racionalizar los subsidios ineficientes a combustibles fósiles que alientan el consumo antieconómico mediante la eliminación de las distorsiones del mercado']
  },
  '32310': {
    'ODS 1 Fin de la pobreza': ['11.C. Asistencia financiera y técnica']
  },
  '33110': {
    'ODS 1 Fin de la pobreza': ['17.10. Promover un sistema de comercio multilateral universal, basado en normas, abierto, no discriminatorio y equitativo'],
    'ODS 8 Trabajo decente y crecimiento económico': ['8.A. Aumentar apoyo a la iniciativa de ayuda para el comercio en los países en desarrollo']
  },
  '33120': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.A. Aumentar apoyo a la iniciativa de ayuda para el comercio en los países en desarrollo']
  },
  '33130': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.A. Aumentar apoyo a la iniciativa de ayuda para el comercio en los países en desarrollo']
  },
  '33140': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.A. Aumentar apoyo a la iniciativa de ayuda para el comercio en los países en desarrollo']
  },
  '33150': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.A. Aumentar apoyo a la iniciativa de ayuda para el comercio en los países en desarrollo']
  },
  '33181': {
    'ODS 8 Trabajo decente y crecimiento económico': ['8.A. Aumentar apoyo a la iniciativa de ayuda para el comercio en los países en desarrollo']
  },
  '33210': {
    'ODS 1 Fin de la pobreza': ['12.B. Elaborar y aplicar instrumentos que permitan seguir de cerca los efectos en el desarrollo sostenible con miras a lograr un turismo sostenible', '14.7. Aumentar los beneficios económicos que los pequeños Estados insulares en desarrollo y los países menos adelantados reciben del uso sostenible de los recursos marinos'],
    'ODS 8 Trabajo decente y crecimiento económico': ['8.9. Políticas turismo sostenible, creen trabajo y promuevan culturas']
  },
  '41010': {
    'ODS 3 Salud y bienestar': ['3.9. Reducir nº muertes/enfermedades por productos químicos']
  },
  '41020': {
    'ODS 1 Fin de la pobreza': ['11.6. Reducir impacto ambiental negativo de las ciudades', '14.1. Prevenir y reducir la contaminación marina', '14.2. Gestionar y proteger de manera sostenible los ecosistemas marinos y costeros', '14.3. Reducir al mínimo los efectos de la acidificación de los océanos', '14.5. Conservar por lo menos el 10% de las zonas costeras y marinas'],
    'ODS 3 Salud y bienestar': ['3.9. Reducir nº muertes/enfermedades por productos químicos']
  },
  '41030': {
    'ODS 1 Fin de la pobreza': ['15.1. Conservación y uso sostenible de ecosistemas terrestres']
  },
  '41040': {
    'ODS 1 Fin de la pobreza': ['11.4. Proteger y salvaguardar patrimonio cultural y natural']
  },
  '41050': {
    'ODS 1 Fin de la pobreza': ['15.3. Luchar contra la desertificación, rehabilitar las tierras y suelos degradados']
  },
  '41082': {
    'ODS 1 Fin de la pobreza': ['14.A. Aumentar los conocimientos científicos']
  },
  '43030': {
    'ODS 1 Fin de la pobreza': ['11.3. Urbanización inclusiva y sostenible, planificación y gestión participativas', '11.7. Proporcionar acceso universal a zonas verdes y espacios públicos seguros, inclusivos y accesibles', '11.A. Apoyar vínculos positivos entre z. urbanas, periurbanas y rurales']
  },
  '43040': {
    'ODS 1 Fin de la pobreza': ['11.A. Apoyar vínculos positivos entre z. urbanas, periurbanas y rurales'],
    'ODS 2 Hambre cero': ['2.A. Más inversiones infraestructura rural, investigación agrícola']
  },
  '43081': {
    'ODS 4 Educación de calidad': ['4.B. Aumentar nº becas para países en desarrollo']
  },
  '60010': {
    'ODS 1 Fin de la pobreza': ['17.4. Ayudar a los países en desarrollo a lograr la sostenibilidad de la deuda a largo plazo']
  },
  '60020': {
    'ODS 1 Fin de la pobreza': ['17.4. Ayudar a los países en desarrollo a lograr la sostenibilidad de la deuda a largo plazo']
  },
  '60030': {
    'ODS 1 Fin de la pobreza': ['17.4. Ayudar a los países en desarrollo a lograr la sostenibilidad de la deuda a largo plazo']
  },
  '60040': {
    'ODS 1 Fin de la pobreza': ['17.4. Ayudar a los países en desarrollo a lograr la sostenibilidad de la deuda a largo plazo']
  },
  '60061': {
    'ODS 1 Fin de la pobreza': ['17.4. Ayudar a los países en desarrollo a lograr la sostenibilidad de la deuda a largo plazo']
  },
  '60062': {
    'ODS 1 Fin de la pobreza': ['17.4. Ayudar a los países en desarrollo a lograr la sostenibilidad de la deuda a largo plazo']
  },
  '60063': {
    'ODS 1 Fin de la pobreza': ['17.4. Ayudar a los países en desarrollo a lograr la sostenibilidad de la deuda a largo plazo']
  },
  '99820': {
    'ODS 4 Educación de calidad': ['4.7. Adquisición conocimientos promover desarrollo sostenible']
  },
  '91010': {
    'ODS 1 Fin de la pobreza': ['17.2. Países desarrollados cumplen compromisos AOD', '17.7. Promover el desarrollo de tecnologías ecológicamente racionales y su transferencia, divulgación y difusión']
  }
}
